...
const { buildWeeklyDigest, renderDigestText, TZ } = require('../lib/digest');
const { postWeeklyDigest } = require('./digestCron');
const { resolveWorkspaceConfig, isValidTimezone, setTimezone, workspaceKey } = require('../lib/configStore');
...
const { CARBON_ESTIMATE_SYSTEM_PROMPT, parseEstimate, splitImpact, impactDots } = require('../lib/carbonEstimate');
...
async function handleLog({ respond, decision, meta, logger }) {
    ...
    try {
        ...
        await respond({
            response_type: 'ephemeral',
            ...buildErrorCard({ title: "Couldn't estimate impact", body: classifyLlmError(err), hint: 'Try again in a moment, or rephrase your decision.' }),
        });
    }
}
...
async function handleGreenlogCommand({ command, ack, respond, client, logger }) {
	...
	try {
		switch (sub) {
            ...
            case 'digest': {
                ...
                if (result.posted) {
                    ...
                    await respond({
                        response_type: 'ephemeral',
                        ...buildErrorCard({ title: 'No digest channel configured', body: 'GreenLog needs a channel to post weekly digests to.', hint: 'Open the GreenLog App Home tab and pick a channel from the dropdown.' }),
                    });
                } else {
                    await respond({
                        response_type: 'ephemeral',
                        ...buildErrorCard({ title: "Couldn't post digest", body: result.error?.message || 'Unknown error.', hint: 'Try `/greenlog digest` again in a few seconds.' }),
                    });
                }
                break;
            }
		...
		await respond({
			response_type: 'ephemeral',
			...buildErrorCard({ title: 'Something went wrong', body: `\`${err.message || err}\``, hint: 'Try `/greenlog ping` to check status. If this persists, check the server logs.' }),
		});
	}
}
...
function registerGreenlogCommand(app) {
    app.command('/greenlog', async ({ command, ack, respond, client, logger }) => {
        await safeHandler('greenlog-command', () =>
            handleGreenlogCommand({ command, ack, respond, client, logger })
        )();
    });
}
