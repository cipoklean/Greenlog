...
async function sendWelcomeDmIfNeeded({ teamId, enterpriseId, userId, client, logger }) {
    try {
        ...
        if (cfg && cfg.onboardingDmSent) return;
        ...
        const tzLine = (tzSource === 'config' || tzSource === 'detected')
        ...
        if (logger && logger.info) logger.info('sent welcome DM to ' + userId + ' for workspace ' + key);
    } catch (err) {
        const msg = err && err.message ? err.message : String(err);
        if (logger && logger.error) logger.error('sendWelcomeDmIfNeeded failed: ' + msg);
    }
}
...
function registerWelcomeActions(app) {
    app.action('welcome_digest_channel_select', async ({ ack, body, action, client, logger }) => {
        ...
        try {
            ...
            const teamId = body && body.team && body.team.id;
            const enterpriseId = (body && body.enterprise && body.enterprise.id) || null;
            const userId = body && body.user && body.user.id;
            const key = workspaceKey({ team_id: teamId, enterprise_id: enterpriseId });
            if (!key) {
                if (logger && logger.warn) logger.warn('welcome_digest_channel_select: no workspace key');
                return;
            }
            ...
            const im = await client.conversations.open({ users: userId });
            console.log('[welcomeAction] conversations.open ok=' + (im && im.ok) + ', channelId=' + (im && im.channel && im.channel.id));
            const dmChannelId = im && im.channel && im.channel.id;
            if (dmChannelId) {
                await client.chat.postMessage({
                    channel: dmChannelId,
            ...
            if (logger && logger.info) logger.info('set digest channel ' + channelId + ' for workspace ' + key);
        } catch (err) {
            const msg = err && err.message ? err.message : String(err);
            if (logger && logger.error) logger.error('welcome_digest_channel_select handler failed: ' + msg);
        }
    });
}
