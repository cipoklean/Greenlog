...
const { countUsageBySource } = require('./usage');
...
function localMidnightUTC(localDateStr, tz = DEFAULT_TZ) {
  ...
  const localAsUTC = Date.UTC(+o.year, +o.month - 1, +o.day, hour, +o.minute, +o.second);
...
async function buildWeeklyDigest({ now = new Date(), channelId, tz = DEFAULT_TZ } = {}) {
  ...
  const summary = aggregateLogs(filtered);
  // Mention decisions are intentionally not logged, but we still count how
  // many times @GreenLog was used this week so the Sources line is meaningful.
  const usage = await countUsageBySource({ startISO, endISO });
  summary.bySource['mention-direct'] = usage['mention-direct'];
  summary.bySource['mention-thread'] = usage['mention-thread'];
  return {
...
function renderDigestText(digest, { channelScoped = false } = {}) {
  ...
  const endStr = toDisplayDateString(new Date(new Date(range.endISO).getTime() - 86400000).toISOString(), tz);
  ...
  lines.push(
    '',
    `_Sources: ${summary.bySource.slash} logged · ${summary.bySource['mention-direct'] + summary.bySource['mention-thread']} mention estimate${summary.bySource['mention-direct'] + summary.bySource['mention-thread'] === 1 ? '' : 's'}_`,
  );
  return lines.join('\n');
}

function renderDigestBlocks(digest, { channelScoped = false, canvasUrl = null } = {}) {
  const { summary, range } = digest;
  const tz = range.tz || DEFAULT_TZ;
  const startStr = toDisplayDateString(range.startISO, tz);
  const endStr = toDisplayDateString(new Date(new Date(range.endISO).getTime() - 86400000).toISOString(), tz);
  ...
  blocks.push({
    ...
    elements: [
      {
        type: 'mrkdwn',
        text: `_Sources: ${summary.bySource.slash} logged · ${summary.bySource['mention-direct'] + summary.bySource['mention-thread']} mention estimate${summary.bySource['mention-direct'] + summary.bySource['mention-thread'] === 1 ? '' : 's'}_`,
      },
    ],
  });
  return blocks;
