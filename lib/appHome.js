      ...
      view: {
        ...
        blocks: [
          ...
          },
          {
            type: 'context',
            elements: [
              { type: 'mrkdwn', text: '🌍 Built for the Slack Agent Builder Challenge — Agent for Good track.' },
            ],
          },
        ],
      },
    });
    if (logger?.info) logger.info(`published App Home for ${userId}`);
  } catch (err) {
    const msg = err?.message ? err.message : String(err);
    if (logger?.error) logger.error(`publishAppHome failed: ${msg}`);
  }
}
...
function registerAppHomeEvents(app) {
  app.event('app_home_opened', async ({ event, body, client, logger }) => {
    try {
      ...
      const msg = err?.message ? err.message : String(err);
      if (logger?.error) logger.error(`app_home_opened handler failed: ${msg}`);
    }
  });
  ...
  app.action('apphome_timezone_select', async ({ ack, body, action, client, logger }) => {
    await ack();
    try {
      const tz = action?.selected_option?.value;
      const { teamId, enterpriseId, userId, key } = actionContextKeys(body);
      if (!tz || !key) return;
      if (!isValidTimezone(tz)) {
        if (logger?.warn) logger.warn(`apphome_timezone_select: invalid tz ${tz}`);
        return;
      }
      await setTimezone(key, tz);
      await publishAppHome({ teamId, enterpriseId, userId, client, logger });
      if (logger?.info) logger.info(`apphome: set tz ${tz} for ${key}`);
    } catch (err) {
      const msg = err?.message ? err.message : String(err);
      if (logger?.error) logger.error(`apphome_timezone_select failed: ${msg}`);
    }
  });
  ...
  app.action('apphome_digest_channel_select', async ({ ack, body, action, client, logger }) => {
    ...
    try {
      ...
      await publishAppHome({ teamId, enterpriseId, userId, client, logger });
      if (logger?.info) logger.info(`apphome: set channel ${channelId} for ${key}`);
    } catch (err) {
      const msg = err?.message ? err.message : String(err);
      if (logger?.error) logger.error(`apphome_digest_channel_select failed: ${msg}`);
    }
  });
  ...
  app.action('apphome_run_digest_now', async ({ ack, body, client, logger }) => {
    ...
    try {
      ...
      if (logger?.info) logger.info(`apphome: ran digest on demand for ${cfg.workspaceKey}`);
    } catch (err) {
      const msg = err?.message ? err.message : String(err);
      if (logger?.error) logger.error(`apphome_run_digest_now failed: ${msg}`);
    }
  });
}
