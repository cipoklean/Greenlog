...
async function readAllConfigs() {
  ...
  try {
    await fs.rename(CONFIG_PATH, corruptPath);
  } catch (_) {}
  return {};
}
...
async function writeAllConfigs(configs) {
  await ensureDataDir();
  await fs.writeFile(CONFIG_PATH, JSON.stringify(configs, null, 2), { encoding: 'utf8', mode: 0o600 });
}
...
async function setDigestChannel(key, channelId) {
  return updateConfig(key, { digestChannel: channelId });
}
async function setTimezone(key, timezone) {
  return updateConfig(key, { timezone });
}
