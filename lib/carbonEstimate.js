...
/**
 * Replace angle brackets with fullwidth equivalents to neutralize
 * Slack mrkdwn syntax (<!channel>, <http://...|...>, <@USER>, <#CHANNEL>)
 * in LLM-generated text. Fullwidth brackets look identical to users
 * but are not recognized as mrkdwn by Slack.
 */
function sanitizeMrkdwn(text) {
...
function parseEstimate(raw) {
  ...
  for (const line of raw.split('\n')) {
    ...
    if (m) {
      out[m[1].toLowerCase()] = sanitizeMrkdwn(m[2]);
    }
  }
  return out;
}
